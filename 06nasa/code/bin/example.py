...
class Main:
    ...
    def __init__(self):
        # src/Main.hx:14
        self.getUrl(("https://api.nasa.gov/planetary/apod?api_key=DEMO_KEY&date=" + HxOverrides.stringOrNull(self.getCurrentDate())),True)
...
class haxe_Http:
    ...
    def request(self,post = None):
        # /usr/local/lib/haxe/std/haxe/Http.hx:187
        _gthis = self
        # /usr/local/lib/haxe/std/haxe/Http.hx:325
        me = self
        # /usr/local/lib/haxe/std/haxe/Http.hx:326
        output = haxe_io_BytesOutput()
        # /usr/local/lib/haxe/std/haxe/Http.hx:327
        old = self.onError
        # /usr/local/lib/haxe/std/haxe/Http.hx:328
        err = False
        # /usr/local/lib/haxe/std/haxe/Http.hx:329
        def _hx_local_0(e):
            # /usr/local/lib/haxe/std/haxe/Http.hx:329
            nonlocal err
            # /usr/local/lib/haxe/std/haxe/Http.hx:333
            me.responseData = output.getBytes().toString()
            # /usr/local/lib/haxe/std/haxe/Http.hx:335
            err = True
            # /usr/local/lib/haxe/std/haxe/Http.hx:337
            _gthis.onError = old
            # /usr/local/lib/haxe/std/haxe/Http.hx:338
            _gthis.onError(e)
        self.onError = _hx_local_0
        # /usr/local/lib/haxe/std/haxe/Http.hx:340
        self.customRequest(post,output)
        # /usr/local/lib/haxe/std/haxe/Http.hx:341
        if (not err):
            # /usr/local/lib/haxe/std/haxe/Http.hx:345
            def _hx_local_1():
                # /usr/local/lib/haxe/std/haxe/Http.hx:345
                me.responseData = output.getBytes().toString()
                return me.responseData
            me.onData(_hx_local_1())

    def customRequest(self,post,api,sock = None,method = None):
        # /usr/local/lib/haxe/std/haxe/Http.hx:365
        self.responseData = None
        # /usr/local/lib/haxe/std/haxe/Http.hx:366
        url_regexp = EReg("^(https?://)?([a-zA-Z\\.0-9_-]+)(:[0-9]+)?(.*)$", "")
        # /usr/local/lib/haxe/std/haxe/Http.hx:367
        url_regexp.matchObj = python_lib_Re.search(url_regexp.pattern,self.url)
        if (url_regexp.matchObj is None):
            # /usr/local/lib/haxe/std/haxe/Http.hx:368
            self.onError("Invalid URL")
            # /usr/local/lib/haxe/std/haxe/Http.hx:369
            return
        # /usr/local/lib/haxe/std/haxe/Http.hx:371
        ...
        if multipart:
            ...
            try:
                ...
                e2 = _hx_e1
                tmp4 = None
            tmp5 = (("null" if tmp3 is None else tmp3) + Std.string(tmp4))
...
class haxe_io_Input:
    ...
    def readByte(self):
        # /usr/local/lib/haxe/std/haxe/io/Input.hx:50
        raise _HxException("Not implemented")

    def readBytes(self,s,pos,_hx_len):
        # /usr/local/lib/haxe/std/haxe/io/Input.hx:62
        k = _hx_len
        # /usr/local/lib/haxe/std/haxe/io/Input.hx:63
        b = s.b
        # /usr/local/lib/haxe/std/haxe/io/Input.hx:64
        if (((pos < 0) or ((_hx_len < 0))) or (((pos + _hx_len) > s.length))):
            raise _HxException(haxe_io_Error.OutsideBounds)
        # /usr/local/lib/haxe/std/haxe/io/Input.hx:66
        try:
            while (k > 0):
                # /usr/local/lib/haxe/std/haxe/io/Input.hx:75
                b[pos] = self.readByte()
                # /usr/local/lib/haxe/std/haxe/io/Input.hx:77
                pos = (pos + 1)
